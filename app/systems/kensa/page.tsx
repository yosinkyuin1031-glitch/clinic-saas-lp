      ...
      <section className="py-16 max-w-5xl mx-auto px-6">
        ...
        <div className="space-y-6 max-w-2xl mx-auto">
          {[
            ...
            {q:"既存の患者データを移行できますか？",a:"CSV取り込みに対応しています。Excel・Googleスプレッドシート・他の検査記録ツールからも、列を合わせれば一括取り込みが可能です。移行作業も無料でサポートします。"},
            {q:"整体院以外でも使えますか？",a:"はい。鍼灸院・接骨院・ボディケアサロンでもご利用いただけます。検査ロジックは姿勢・歪み評価をベースにしているので、業種を問わず使えます。"},
            {q:"検査スキルが浅いスタッフでも使えますか？",a:"はい。3つのランドマーク（乳様突起・肩甲下角・腸骨稜）を3択で選ぶだけで原因部位が自動判定される設計です。検査ロジックがアプリ側に組み込まれているので、新人スタッフでも院長と同じ精度で検査ができます。"},
            {q:"「業界唯一」の段階的原因特定ロジックって何が違うんですか？",a:"一般的な姿勢評価は「歪んでいる場所」を指摘するだけですが、カラダマップは立位→座位→上半身と段階的にチェックすることで「足由来か / 骨盤由来か / 上半身由来か」を切り分けます。神経整体の現場ロジックをそのままアプリ化したのは現時点で他に確認できていません。"},
            {q:"AIが提案するセルフケアは安全ですか？",a:"はい。原因部位ごとに実際の臨床で使われている定型セルフケア（足関節回旋・タオルギャザー・カーフレイズ等）からAIが組み合わせる設計です。完全自動生成ではなく、安全性が担保されたプールから提案されます。"},
          ].map(({q,a})=>(
